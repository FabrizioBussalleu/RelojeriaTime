...
- **Compras al por mayor (`/por-mayor`):** muestra los relojes publicados más los marcados como *solo al por mayor* (`products.wholesale_only`), sin precios. El cliente elige cantidades y la lista se envía como cotización por WhatsApp; nada se cobra en línea. Tiene su propio orden (`wholesale_position`), que se acomoda en la pestaña "Al por mayor" del organizador. Un reloj *solo al por mayor* desaparece del catálogo de la tienda y su ficha no muestra precio ni carrito.
- **Pago contra entrega:** el checkout, la página del pedido y el correo explican que el pago se coordina por WhatsApp, adelantado o al recibir. El pedido queda en "pendiente de pago" hasta que el panel lo confirme, y el cron lo libera si nadie responde en `pending_order_ttl_hours`.
...
- **Organizador:** fija el orden de la tienda arrastrando las tarjetas (en el celular, mantener presionado; con teclado, espacio y flechas). Los agotados se ven en gris pero conservan su posición: así queda lista para cuando vuelva el stock.
...
- **Catálogo de la tienda sin agotados:** la home lista solo los relojes con stock (ver un reloj que ya no está desanima la compra). Su ficha sigue abriéndose por enlace directo, con el cartel de agotado, y "Compras al por mayor" los sigue mostrando.
- **Eliminar un pedido** (`delete_order`) lo borra con sus productos e historial y devuelve el stock, salvo que ya estuviera cancelado. Es para los pedidos creados por error; un pedido real que no se concretó se cancela, para que quede el registro.
